...
### 1. Install Package (Optional but Recommended)
```bash
pip install -e .
```

This allows you to use the package from anywhere.
...
### Reports
- `data/reports/pdfs/ET_Intelligence_Brief_[timestamp].pdf` - Professional PDF
- `data/reports/pdfs/intelligence_brief_[timestamp].json` - Raw data
- `data/reports/charts/` - Visualization images
...
2. **Tuesday (Processing)**
   ```bash
   python -m et_intel.cli.cli --import comments.csv --platform instagram --subject "Taylor Swift"
   # Or use interactive mode: python -m et_intel.cli.cli
...
4. **Thursday (Distribution)**
   - Review PDF report in `data/reports/pdfs/`
...
You need to import CSV files first using the CLI import command or Option 1 in interactive mode.
